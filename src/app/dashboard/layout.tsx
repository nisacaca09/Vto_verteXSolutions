...
import { Monitor, LayoutDashboard, FolderKanban, User, LogOut, Menu, Search, ChevronLeft } from "lucide-react";
    ...
    <div className="flex h-screen bg-[var(--vto-bg)]">
      ...
      <aside className={cn(
        ...
        <div className="flex h-16 items-center justify-between border-b border-[var(--vto-border)] px-4">
          {!collapsed && (
            <Link href="/dashboard" className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--vto-purple)]">
                <Monitor className="h-4 w-4 text-white" />
              </div>
              <div>
                <span className="font-sans text-sm font-bold text-white">VTO</span>
                <span className="ml-1 text-[8px] font-mono text-[var(--vto-text-dim)]">Client</span>
              </div>
            </Link>
          )}
          {collapsed && (
            <div className="mx-auto flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--vto-purple)]">
              <Monitor className="h-4 w-4 text-white" />
            </div>
          )}
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-4 space-y-1">
          {sidebarLinks.map((link) => (
            <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className={cn(
              ...
              pathname === link.href
                ? "bg-[var(--vto-purple)]/10 text-[var(--vto-purple)] border border-[var(--vto-purple)]/20"
        ...
        <div className="border-t border-[var(--vto-border)] p-2">
          <Link href="/login" className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-xs font-mono text-[var(--vto-text-dim)] hover:bg-[var(--vto-bg-elevated)] hover:text-white", collapsed && "justify-center px-2")}>
      ...
      <main className={cn("flex-1 overflow-y-auto transition-all duration-300", collapsed ? "md:ml-16" : "md:ml-56")}>
        <header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-[var(--vto-border)] bg-[var(--vto-bg)]/80 backdrop-blur-xl px-4 lg:px-6">
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Cari proyek saya..." className="w-48 lg:w-64 rounded-lg border border-[var(--vto-border)] bg-[var(--vto-bg-card)] py-1.5 pl-9 pr-3 text-xs font-mono text-white placeholder:text-[var(--vto-text-dim)] focus:border-[var(--vto-purple)] focus:outline-none transition-all" />
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--vto-purple)]/15 border border-[var(--vto-purple)]/30 text-xs font-sans font-bold text-[var(--vto-purple)]">JD</div>
            <div className="hidden sm:block">
              <div className="text-xs font-sans font-medium text-white">John Doe</div>
              <div className="text-[10px] font-mono text-[var(--vto-text-dim)]">PT Bank Nusantara</div>
            </div>
          </div>
        </header>
